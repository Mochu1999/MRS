...
Shader::ShaderProgramSource Shader::parseShader(const string& filepath) 
{
	...
	while (getline(stream, line)) 
	{
		...
		{
			//npos is the constant that will be given if it was not found, otherwise it will give a position, not a bool
			...
			{ //what isn't in #shader line is included in the stream
...
//p3
void Shader::setUniform(const std::string& name, const p3& vec) {
...
void Shader::setUniform(const std::string& name, p3 vec, float a)
{
	glUniform4f(getUniformLocation(name), vec.x, vec.y, vec.z, a);
}
//1i
...
//mat4
void Shader::setUniform(const std::string& name, const std::array<float, 16>& mat) {
