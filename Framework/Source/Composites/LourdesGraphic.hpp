...
struct LourdesGraphic
{
	...
	p3 shipHeave = { 0,0.65,0 }; float shipHeaveIncrease = 0.0006;
	...
	void draw()
	{
		...
		if (shipHeave.y >= 0.7 || shipHeave.y <= 0.6)
...
struct Lourdes
{
	...
	Lourdes(Shader& shader3D_, Camera& camera_, GlobalVariables gv) :shader3D(shader3D_), camera(camera_)
	{
